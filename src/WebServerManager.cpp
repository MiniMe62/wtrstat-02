...
<body>
            ...
            <div class="sunshine-bar-wrapper" id="sunshineBarWrapper">
                <div class="sunshine-bar-title">
                    <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                        <span id="sunshineBarTitleText">⏱️ Heliografická Lišta Slnečného Svitu (Dnes: 00:00 – 24:00)</span>
                        <span id="sunshineBarDetail" style="display: none; font-size: 0.78rem; font-weight: 600; color: #38bdf8; background: rgba(56, 189, 248, 0.12); border: 1px solid rgba(56, 189, 248, 0.3); padding: 2px 8px; border-radius: 6px; transition: all 0.2s ease;"></span>
                    </div>
                ...
                <div class="sunshine-ticks" id="sunshineTicks">
                    <span>00:00</span>
                    <span>04:00</span>
                    <span>08:00</span>
                    <span>12:00</span>
    ...
    <script>
        ...
        let liveLightSamples = [];

        let activeLightLabels = [...liveLightLabels];
        let activeLightData = [...liveLightSeries];
        let activeLightSamples = [];

        // Predvyplnenie časových značiek pre východzie live vzorky
        const _initNow = new Date();
        liveLightLabels.forEach((lbl, idx) => {
            const parts = lbl.split(':');
            const h = parseInt(parts[0], 10);
            const m = parseInt(parts[1], 10);
            const d = new Date(_initNow.getFullYear(), _initNow.getMonth(), _initNow.getDate(), h, m, 0);
            liveLightSamples.push({ date: d, val: liveLightSeries[idx] });
        });
        activeLightSamples = [...liveLightSamples];
        ...
        function updateSunshineTimeline(labels, data) {
            ...
            const ticksEl = document.getElementById('sunshineTicks');
            const titleEl = document.getElementById('sunshineBarTitleText');
            const detailEl = document.getElementById('sunshineBarDetail');
            const totEl = document.getElementById('sunshineTimelineTotal');
            const statSun = document.getElementById('statLightSunshine');

            const now = new Date();
            const period = lightPeriod || '24h';

            // Počet kalendárnych dní podľa zvoleného obdobia
            let numDays = 1;
            if (period === '3d') numDays = 3;
            else if (period === '7d') numDays = 7;

            // Titulok lišty
            if (titleEl) {
                if (numDays === 1) {
                    titleEl.innerText = '⏱️ Heliografická Lišta Slnečného Svitu (Dnes: 00:00 – 24:00)';
                } else {
                    titleEl.innerText = `⏱️ Heliografická Lišta Slnečného Svitu (${numDays} dni po dňoch: 00:00 – 24:00)`;
                }
            }

            // Generovanie zoznamu dní (od najstaršieho po dnešok)
            const days = [];
            ...
            const samples = (activeLightSamples && activeLightSamples.length > 0) ? activeLightSamples : [];
            ...
            days.forEach((dayStart, dayIdx) => {
                const isToday = (dayIdx === days.length - 1);
                const dateStr = `${dayNamesShort[dayStart.getDay()]} ${dayStart.getDate()}.${dayStart.getMonth() + 1}.`;

                // Červená ryska o polnoci medzi dňami (pred začiatkom dňa 1..N)
                if (dayIdx > 0) {
                    const divider = document.createElement('div');
                    divider.className = 'sunshine-divider-midnight';
                    divider.title = `Polnoc 00:00 (${dateStr})`;
                    bar.appendChild(divider);
                }

                // 1. Krok: Predbežné vyhodnotenie všetkých 96 slotov dňa
                const daySlots = [];
                for (let s = 0; s < 96; s++) {
                    const slotStartTime = new Date(dayStart.getTime() + s * 15 * 60 * 1000);
                    const slotEndTime = new Date(slotStartTime.getTime() + 15 * 60 * 1000);
                    const slotMidTime = new Date(slotStartTime.getTime() + 7.5 * 60 * 1000);

                    // Kontrola budúcnosti (iba pre dnešný deň po aktuálnom čase)
                    if (isToday && slotStartTime.getTime() > now.getTime()) {
                        daySlots.push({ cond: 'future', val: null, startTime: slotStartTime, endTime: slotEndTime });
                        countFutureSlots++;
                        continue;
                    }

                    // Vyhľadanie najlepšej vzorky pre tento slot (okno ±12 minút od stredu slotu)
                    let bestSample = null;
                    let minDiff = 12 * 60 * 1000;

                    for (let k = 0; k < samples.length; k++) {
                        const samp = samples[k];
                        const diff = Math.abs(samp.date.getTime() - slotMidTime.getTime());
                        if (diff < minDiff) {
                            minDiff = diff;
                            bestSample = samp;
                        }
                    ...
                    if (bestSample) {
                        ...
                        // Nočné hodiny bez záznamu (pred 05:30 alebo po 21:00) sú bezpečne Noc (0 %)
                        const timeInMin = slotStartTime.getHours() * 60 + slotStartTime.getMinutes();
                        if (timeInMin < 5 * 60 + 30 || timeInMin >= 21 * 60) {
                            v = 0;
                        }
                    }
                    ...
                    daySlots.push({ cond: cond, val: v, startTime: slotStartTime, endTime: slotEndTime });
                }

                // 2. Krok: Zlúčenie súvislých slotov s rovnakým typom počasia (Run-Length Encoding)
                const intervals = [];
                let currentInterval = null;

                for (let s = 0; s < daySlots.length; s++) {
                    const slot = daySlots[s];
                    if (currentInterval && currentInterval.cond === slot.cond) {
                        currentInterval.slotsCount++;
                        currentInterval.endTime = slot.endTime;
                        if (slot.val !== null && !isNaN(slot.val)) currentInterval.vals.push(slot.val);
                    } else {
                        ...
                        currentInterval = {
                            ...
                            vals: (slot.val !== null && !isNaN(slot.val)) ? [slot.val] : []
                        };
                    }
                }
                if (currentInterval) intervals.push(currentInterval);

                // 3. Krok: Vykreslenie zlúčených segmentov s proporčným flex a detailmi
                intervals.forEach(inv => {
                    const seg = document.createElement('div');
                    seg.className = 'sunshine-segment ' + inv.cond;
                    seg.style.flex = inv.slotsCount;

                    const durMinutes = inv.slotsCount * 15;
                    const durH = Math.floor(durMinutes / 60);
                    const durM = durMinutes % 60;
                    const durStr = durH > 0 ? (durM > 0 ? `${durH}h ${durM}m` : `${durH}h`) : `${durM}m`;

                    const startStr = inv.startTime.toLocaleTimeString('sk-SK', { hour: '2-digit', minute: '2-digit' });
                    const endStr = inv.endTime.toLocaleTimeString('sk-SK', { hour: '2-digit', minute: '2-digit' });

                    let icon = '☀️';
                    let labelText = 'Priame slnko (Svit)';
                    if (inv.cond === 'cloudy') { icon = '⛅'; labelText = 'Polooblačno'; }
                    else if (inv.cond === 'overcast') { icon = '☁️'; labelText = 'Zamračené'; }
                    else if (inv.cond === 'overcast-dark') { icon = '🌧️'; labelText = 'Husto zamračené'; }
                    else if (inv.cond === 'night') { icon = '🌙'; labelText = 'Noc / Tma'; }
                    else if (inv.cond === 'future') { icon = '⏱️'; labelText = 'Bez dát / Budúcnosť'; }
                    else if (inv.cond === 'nodata') { icon = '❓'; labelText = 'Bez záznamu'; }

                    let valText = '';
                    if (inv.vals.length > 0) {
                        const avgVal = Math.round(inv.vals.reduce((a, b) => a + b, 0) / inv.vals.length);
                        valText = ` • ${avgVal}%`;
                    }

                    const compactTooltip = `${dateStr} ${startStr} – ${endStr} • ${labelText} (${durStr}${valText})`;
                    seg.title = compactTooltip;

                    const detailText = `${icon} ${dateStr} ${startStr} – ${endStr} • ${labelText} (${durStr}${valText})`;
                    const showDetail = () => {
                        if (detailEl) {
                            detailEl.innerText = detailText;
                            detailEl.style.display = 'inline-block';
                            if (window._sunDetailTimer) clearTimeout(window._sunDetailTimer);
                            window._sunDetailTimer = setTimeout(() => {
            ...
            bar.onmouseleave = () => {
                if (detailEl && !window._sunDetailTimer) detailEl.style.display = 'none';
            };
                ...
                if (numDays === 1) {
                    ...
                    days.forEach((d, idx) => {
                        if (idx > 0) {
                            ...
                            sep.style.color = '#ef4444';
                            sep.style.fontWeight = '700';
                            sep.title = 'Polnoc (00:00)';
                            sep.innerText = '● 00:00';
                            ticksEl.appendChild(sep);
                        }
                        const sp = document.createElement('span');
                        const isToday = (idx === days.length - 1);
                        const dayStr = `${dayNamesShort[d.getDay()]} ${d.getDate()}.${d.getMonth() + 1}.`;
                        sp.innerText = isToday ? `Dnes (${dayStr})` : dayStr;
                        if (isToday) {
            ...
            // Súčet dĺžky slnečného svitu (1 slot = 15 minút)
            const todayMins = todaySunSlots * 15;
            const totalMins = totalSunSlots * 15;
            const todayFormatted = `${Math.floor(todayMins / 60)}h ${(todayMins % 60).toString().padStart(2, '0')}m`;
            const totalFormatted = `${Math.floor(totalMins / 60)}h ${(totalMins % 60).toString().padStart(2, '0')}m`;

            if (totEl) {
                if (numDays === 1) {
                    totEl.innerText = `Dnes: ${todayFormatted}`;
                } else {
                    totEl.innerText = `Spolu: ${totalFormatted} (Dnes: ${todayFormatted})`;
                }
            }

            if (statSun) {
                statSun.innerText = todayFormatted;
            }
        }
        ...
        function setLightPeriod(period) {
            ...
            if (period === 'live') {
                ...
                let resultsCount = 110; // 24h
                if (period === '3d') resultsCount = 320;
                if (period === '7d') resultsCount = 720;
        ...
        async function fetchThingSpeakLightHistory(resultsCount) {
            ...
            try {
                ...
                for (const feed of data.feeds) {
                    ...
                    const val = (lVal !== null && !isNaN(lVal)) ? lVal : 0;
                    labels.push(label);
                    lightSeries.push(val);
                    samples.push({ date: d, val: val });
                }
