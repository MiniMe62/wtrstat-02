    ...
    <script>
        ...
        function updateSunshineTimeline(labels, data) {
            ...
            days.forEach((dayStart, dayIdx) => {
                ...
                // Každý deň má 96 slotov po 15 minút (00:00 až 23:45)
                for (let s = 0; s < 96; s++) {
                    const slotStartTime = new Date(dayStart.getTime() + s * 15 * 60 * 1000);
                    const slotMidTime = new Date(slotStartTime.getTime() + 7.5 * 60 * 1000);
                    const timeLabel = slotStartTime.toLocaleTimeString('sk-SK', { hour: '2-digit', minute: '2-digit' });
                    ...
                    const valDisplay = (v !== null && !isNaN(v)) ? `${v.toFixed(0)}%` : '--%';
                    seg.title = `${dateStr} ${timeLabel}: ${valDisplay} (${labelText})`;
                    bar.appendChild(seg);
                }
            });
